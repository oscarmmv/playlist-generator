...
xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
            ...
            CLIENT_ID = config.SPOTIFY_API_CLIENT;
            ...
            async function getSimilarSongs() {
                ...
                if (similarSongs.length > 0) {
                    ...
                    similarSongs.forEach(song => {
                        ...
                        item.innerHTML = `
  <img id="song-art" onclick="window.open('${song.uri}');" src="${song.image}" alt="${song.name} image">
...
`;
            ...
            function eventButtonClick() {
                if (document.getElementById('song-input').value != 0) {
                    getSimilarSongs();
                }
            }
            // Add event listeners to the buttons
            document.getElementById('add-preference-button').addEventListener('click', addPreferenceInput);
            document.getElementById('get-similar-songs-button').addEventListener('click', eventButtonClick);
